...
Phase 7 adds the consulting output layer.  No new analytical engines; the goal
is to make every existing result visible and explainable to non-technical
stakeholders.

New modules:

```
visualization.py   Mermaid flowchart + plain-text workflow graph, compare_text/compare_mermaid
waterfall.py       ROI waterfall decomposition (text + SVG) showing where savings come from
heatmap.py         Bottleneck heatmap (text + SVG) scoring each node across 4 pressure dimensions
snapshot.py        Executive snapshot: concise one-page summary (text + HTML)
assumptions.py     AssumptionProfile: JSON-serializable run configuration with validation
packet.py          consultant_packet: single-command directory export of all artifacts
```

New CLI commands: `visualize-workflow`, `roi-waterfall`, `bottleneck-heatmap`,
`executive-snapshot`, `consultant-packet`, `generate-example-gallery`.

New documentation: `docs/examples_gallery.md`, `docs/api_reference.md`.

New example outputs under `examples/outputs/` (deterministic, regenerated by
`generate-example-gallery`).

Assumption profiles under `examples/data/assumptions_*.json` for quick
conservative/base/aggressive sensitivity checks.
